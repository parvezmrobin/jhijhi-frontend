...
class PlayerDetails extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.unlisten = history.listen((location) => {
      ...
      const playerId = location.pathname.substring(13);
      this._loadPlayer(playerId);
    });

    this._loadPlayer(match.params.id);
    this._loadPlayers();
  }
  ...
  render() {
    const {
      player,
      stat: { bat: battingStat, bowl: bowlingStat, numMatch },
      players,
    } = this.state;
    let mainContent;
    if (!player.name) {
      ...
      mainContent = (
        <main className="col">
          <CenterContent col="col-lg-6 col-md-8">
            ...
            <h5 className="text-center font-weight-normal pb-3">
                ...
                <tr>
                  ...
                  {!battingStat.numInnings && (
                    <th
                      ...
                      rowSpan="5"
                      colSpan="2"
                      className="fs-2 font-weight-normal"
                    >
                      Never Batted
                    </th>
                  )}
                ...
                <tr>
                  {battingStat.numInnings > 0 && (
                    <>
                      ...
                      <td>
                        {!Number.isFinite(battingStat.avgRun)
                          ? 'N/A'
                          : Math.round(battingStat.avgRun * 100) / 100}
                      </td>
                    </>
                  )}
                  {bowlingStat.numInnings > 0 && (
                    <>
                      ...
                      <td>
                        {!Number.isFinite(bowlingStat.avgWicket)
                          ? 'N/A'
                          : Math.round(bowlingStat.avgWicket * 100) / 100}
                      </td>
                    </>
                  )}
                </tr>
                <tr>
                  ...
                  {bowlingStat.numInnings > 0 && (
                    <>
                      ...
                      <td>
                        {Number.isInteger(bowlingStat.bestFigure.run) &&
                          `${bowlingStat.bestFigure.wicket}-${bowlingStat.bestFigure.run}`}
                      </td>
                    </>
                  )}
                </tr>
                <tr>
                  {battingStat.numInnings > 0 && (
                    <>
                      ...
                      <td>
                        {!Number.isFinite(battingStat.strikeRate)
                          ? 'N/A'
                          : Math.round(battingStat.strikeRate * 100) / 100}
                      </td>
                    </>
                  )}
                  {bowlingStat.numInnings > 0 && (
                    <>
                      ...
                      <td>
                        {!Number.isFinite(bowlingStat.strikeRate)
                          ? 'N/A'
                          : Math.round(bowlingStat.strikeRate * 100) / 100}
                      </td>
                    </>
                  )}
                </tr>
